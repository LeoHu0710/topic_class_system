...
## 由於LLM有不確定性，如果需要完全固定輸出 genai.GenerativeModel() 可以將 temperature 可以設置為0
...
async def topic_class_system_function(json_data : UploadFile = File(...)):
    ...
    try :
        ...
        logger.info(f"topic_info: {topic_info[:3]}")

        product_analyst_prompt = prompt.product_analyst_prompt
        topic_info_prompt = f"""
        ---
        The original input (topic_info) is as follows:
        {topic_info}
        ---
        """
        logger.info("Sending prompt to LLM")
        response = await llmmodel.generate_content_async(product_analyst_prompt + topic_info_prompt)
        ## 檢查返回結果是否有 text 屬性
        if hasattr(response, 'text'):
            response_strip = response.text.strip()
        logger.info(f"Success!\nResponse = {response_strip}")
